...
- [ ] **잉크 리플 (Ink Ripple)** — 배지 없이 커서 지점에서 얇은 링이 물결처럼 한 번 퍼지고 소멸 (~0.4s). 링 반경 0→48pt 확장 + 두께 감소 + 페이드, 중앙에 작은 체크 잔상. 가장 미니멀한 스타일.
...
- [ ] 스타일 선택 UI: 설정에 스타일 피커 추가 + 선택 시 샘플 HUD 1회 표시 (위치 미리보기 항목과 같은 메커니즘 공유)
